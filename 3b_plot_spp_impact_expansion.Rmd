...
```{r expansion by species, eval = FALSE}
...
tr_by_priority <- tr_by_stressor_spp %>%
  ...
  left_join(imp_range_df %>%
              filter(year == 2013 & stressor == 'cum_all') %>%
              select(iucn_sid, pct_impact, all_r_km2),
            by = 'iucn_sid') %>%
  filter(term == 'year') %>%
...
ggplot(tr_by_priority, aes(x = priority, y = estimate)) +
  ...
  scale_x_log10() +
  facet_wrap( ~ stressor) +
  labs(title = 'expansion by priority')

ggplot(tr_by_priority, aes(x = 1/log(all_r_km2), y = estimate)) +
  geom_point(aes(color = sig)) +
  scale_x_log10() +
  facet_wrap( ~ stressor) +
  labs(title = 'expansion by total range')

ggplot(tr_by_priority, aes(x = pct_impact, y = estimate)) +
  geom_point(aes(color = sig)) +
  scale_x_log10() +
  facet_wrap( ~ stressor) +
  labs(title = 'expansion by pct impacted range')
```

But does this really show anything? since pct_impacted is a late value, a small pct impacted must almost certainly be an impact that remained small throughout the time series - therefore would have a tiny slope.
