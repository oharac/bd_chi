...
imp_full_range_plot <- ggplot(cum_range, aes(x = code)) +
  ...
  geom_boxplot(aes(y = imp_range_pct), 
...
taxon_incl_plot <- ggplot(taxon_pct_range_risk, aes(x = code, y = pct)) +
  ggtheme_plot() +
  geom_col(aes(fill = desc), color = 'white', size = .25) +
  scale_fill_manual(values = taxon_colors$fill) +
  theme(legend.position = 'bottom',
        strip.text = element_blank(),
        strip.background = element_blank(),
        legend.key.size = unit(.3, 'cm')) +
  geom_text(data = taxon_n_labels, y = .06, aes(label = n_lbl),
...
# 
x <- ggdraw() +
  draw_plot(imp_full_range_plot, x = 0, y = .64, width = 1, height = .36) +
...
ggsave(plot = x, filename = 'si_compare_imp_range_to_risk.png', 
       height = 6.5, width = 6.5)
knitr::include_graphics('si_compare_imp_range_to_risk.png')
```
