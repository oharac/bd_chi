...
if(!file.exists(fig1_file)) {
  ...
  fig1a_panel <- get_panel(fig1a_impact_map)
  fig1b_panel <- get_panel(fig1b_intens_map)
  fig1a_legend <- get_legend(fig1a_impact_map)
  fig1b_legend <- get_legend(fig1b_intens_map)
  map_width <- .9
  fig1 <- ggdraw() +
    draw_plot(fig1a_panel,  x = .01, y = 0.51, width = .88, height = .48) +
    draw_plot(fig1b_panel,  x = .01, y = 0.01, width = .88, height = .48) +
    draw_plot(fig1a_legend, x = .9, y = 0.5, width = .1, height = .5) +
    draw_plot(fig1b_legend, x = .9, y = 0,   width = .1, height = .5) +
    ...
    draw_label('Impact score', x = .935, y = .93, hjust = 1, vjust = 1, 
               size = 7, color = 'grey20', fontface = 'bold') +
    draw_label('Intensification', x = .93, y = .43, hjust = 1, vjust = 1, 
               size = 7, color = 'grey20', fontface = 'bold')
...
if(!file.exists(figS1_file)) {
  ...
  n_imp_map <- ggplot() +
    ggtheme_map(base_size = 7) +
    geom_raster(data = imp_ct_df, aes(x, y, fill = n)) +
    geom_sf(data = land_sf, fill = 'grey96', color = 'grey40', size = .10) +
  ...
  nspp_map <- ggplot() +
    ggtheme_map(base_size = 7) +
    geom_raster(data = nspp_df, aes(x, y, fill = nspp), size = .1) +
    geom_sf(data = land_sf, fill = 'grey96', color = 'grey40', size = .10) +
    scale_fill_viridis_c(breaks = lgd_brks, labels = lgd_brks) +
    theme(plot.margin = unit(c(.05, 0, .05, 0), units = 'cm'),
          legend.key.width = unit(.25, 'cm')) +
...
if(!file.exists(figS1_file)) {
  figS1 <- plot_grid(n_imp_map, nspp_map, align = 'v',
                    nrow = 2, labels = c('A', 'B'), label_size = 9)
