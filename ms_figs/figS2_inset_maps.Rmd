...
if(!file.exists(fig_file)) {
  ...
  row_title_w <- .10
  inset_gap <- 0.02
  inset_w <- (1 - legend_w - row_title_w - n_insets * inset_gap) / n_insets
  inset_h <- inset_w
  inset_corner_x <- row_title_w + (inset_gap + inset_w) * 0:(n_insets-1)
  inset_corner_y <- inset_gap + (inset_gap + inset_h) * 1:0
  name_pos_y <- 2.5 * inset_gap + 2 * inset_h
  ...
  map_w_insets <- 
    ...
    draw_plot(imp_legend, x = .89, y = inset_corner_y[1], width = .1, height = inset_h) +
    draw_plot(int_legend, x = .89, y = inset_corner_y[2], width = .1, height = inset_h) +
    draw_label('Western Mexico', x = name_pos_x[1], y = name_pos_y, 
               vjust = 0, hjust = .5, size = 8, color = 'grey20', fontface = 'bold') +
    draw_label('Northern Venezuela', x = name_pos_x[2], y = name_pos_y, 
               vjust = 0, hjust = .5, size = 8, color = 'grey20', fontface = 'bold') +
