...
intens_lvls <- c('> 1.0%/yr', '> 0.1%/yr', '> 0%/yr', 
                 'no change', 
                 '< 0%/yr', '< -0.1%/yr', '< -1.0%/yr')
plot_tax_sum_df <- tax_sum_df %>%
  ...
  gather(var, val, mean_pct_incr1:mean_pct_decr3, mean_pct_nochange) %>%
...
fig2 <- plot_grid(fig2a + theme(legend.position = 'none'), ### drop legend for manual placement
  ...
  draw_plot(legend_2b, x = .72, y = .2, width = .2, height = .4) +
