...
if(reload == FALSE) {
  maps_already_rasterized <- list.files(file.path(dir_bd_anx, 'spp_rasts_mol_2019'),
                                        pattern = '.csv') %>%
    str_extract('[0-9]+') %>%
    as.integer()
...
# zxcv <- list.files(file.path(dir_bd_anx, 'spp_rasts_mol_2019'), 
#                    full.names = TRUE, 
...
if(nrow(maps_to_rasterize) == 0) { ### all maps accounted for as .csvs
  ...
  for(i in seq_along(shps)) {
    ...
    #   # st_write(polys_match, file.path(dir_bd_anx, 'reptiles_shp_buffered.shp'))
    #   # dbf <- foreign::read.dbf(file.path(dir_bd_anx, 'reptiles_shp_buffered.dbf'))
    #   # foreign::write.dbf(dbf, file.path(dir_bd_anx, 'reptiles_shp_buffered.dbf'))
    #   
    #   polys_match <- st_read(file.path(dir_bd_anx, 'reptiles_shp_buffered.shp')) %>%
    ...
    # system.time({
      tmp <- parallel::mclapply(seq_along(spp_ids),
        ...
        # rast_file <- file.path(dir_bd_anx, 'spp_rasts_mol_2019',
        ...
        write_csv(spp_present, file.path(dir_bd_anx, 'spp_rasts_mol_2019',
...
csvs <- list.files(file.path(dir_bd_anx, 'spp_rasts_mol_2019')) %>%
  str_extract('[0-9]+')

x <- list.files(file.path(dir_bd_anx, 'spp_rasts_mol_2019'),
                full.names = TRUE)
