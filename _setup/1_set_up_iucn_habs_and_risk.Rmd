...
spp_info_from_api_file <- file.path(dir_o_anx, sprintf('iucn/spp_info_from_api_%s.csv',
                                                       api_version))
reload <- FALSE
  ...
  for(j in 1:n_chunks) { 
    ...
    if(!file.exists(chunk_file)) {
      ...
      spp_habs_chunk <- mc_get_from_api(spp_habs_url, spp_ids_chunk, api_key, 
                                        cores = 12, delay = .5, verbose = FALSE)
...
### 21 spp
write_csv(marg_suit, 
          file.path(dir_setup, 'int', sprintf('spp_marine_marginal_%s.csv', api_version)))
...
if(!file.exists(spp_timeseries_file) | reload) {
  ...
  for(j in 1:n_chunks) { 
    ...
    if(!file.exists(chunk_file)) {
      cat_msg('Getting past assessment info for species ', min(spp_index), ' to ', max(spp_index))
      ...
      cat_msg('... found ', nrow(spp_past_chunk), ' past assessment rows for these species')
  ...
  spp_past_chunk_files <- list.files(file.path(dir_o_anx, 'tmp'), 
...
if(!file.exists(spp_current_file) | reload) {
  ...
  ptm <- proc.time()
  spp_current_df <- mc_get_from_api(spp_curr_url, sid_list, api_key, 
                                    delay = .5, cores = 16, verbose = FALSE)
