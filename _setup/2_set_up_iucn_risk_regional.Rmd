...
rgn_spp_info_file <- here(sprintf('_data/iucn_risk_rgn_current_%s.csv', api_version))
...
if(!file.exists(rgn_spp_info_file)) {
  ...
  spp_rgn_marine <- read_csv(here('_data',
...
global_marine_spp_risk <- read_csv(here(sprintf('_data/iucn_risk_current_%s.csv', api_version)))

rgn_marine_spp <- read_csv(rgn_spp_info_file)

rgn_spp_list <- global_marine_spp_risk %>%
  select(iucn_sid, sciname, comname, global_cat = cat, global_cat_score = cat_score) %>%
...
spp_timeseries_file <- here('_data',
