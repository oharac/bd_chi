...
For each stressor, choose a mid year: 2010 (arbitrary).  Map the 95% contour volume map against the full map - setting values for the full map to one (normalized) value and the values for the 95% contour volume to another to show contrast.  Because all the 95% contour volume cells are within the full raster, we can simply plot it in a different color on top of the full raster.
...
str_95cv_files <- list.files(file.path(dir_bd_anx, 'layers/stressors_10km_95cv'),
                             full.names = TRUE)
# slr_95cv_files <- str_95cv_files[str_detect(str_95cv_files, 'slr_')]
# unlink(slr_95cv_files)

str_full_files <- str_df$file
stressors <- str_df$stressor %>% unique() %>% sort()
