...
if(!file.exists(comp_file)) {
  ...
  comp_spp <- mc_get_from_api(comp_spp_url, comp_gps$group_name, api_key, 
                              add_param_val = TRUE) %>%
    select(iucn_sid = taxonid, sciname = scientific_name, taxon = param_val)
  ...
  syngnathids <- spp_info %>%
    ...
    mutate(taxon = 'syngnathiformes')
  clupeids <- spp_info %>%
    filter(tolower(order) == 'clupeiformes') %>%
    mutate(taxon = 'clupeiformes')
...
* species is listed in a taxon with comprehensively assessed status (incl manually adjusted ones)
