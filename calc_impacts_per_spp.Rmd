...
```{r}
spp_sensitivity <- read_csv(here('_output', 
...
tmp <- parallel::mclapply(spp_ids, mc.cores = 8, 
  FUN = function(spp_id) {
    ...
    if(!file.exists(spp_impact_file) | reload) {
      ...
      spp_range_df <- read_csv(file.path(dir_bd_anx, 'spp_rasts_mol_2019',
      ...
      spp_intsx_files <- spp_str_intsx_df %>%
        ...
        .$intsx_file
      spp_intsx_df <- mclapply(spp_intsx_files, mc.cores = 12, FUN = read_csv) %>%
        bind_rows() %>%
