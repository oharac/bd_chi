...
Here we plot species impact trends - how impacted range area is changing over time.  We can look at this on a stressor-by-stressor basis, and grouped by cumulative stressors by category.  
...
Here we will not summarize by every stressor, but just the cumulative category levels.  The priority will be based on the average impact over the most recent three years of impacts.

``` {r}
priority_by_stressor <- imp_range_df %>%
  filter(!is.na(year)) %>%
  filter(str_detect(stressor, 'cum_')) %>%
  mutate(priority = pct_impact / log(range_km2)) %>%
  group_by(stressor, iucn_sid, taxon) %>%
  arrange(desc(year)) %>%
  mutate(yearnum = 1:n()) %>%
  filter(yearnum <= 3) %>%
...
write_csv(priority_by_stressor, here('_output/spp_priority_by_stressor.csv'))
```
...
tr_by_priority <- tr_by_stressor_spp %>%
  left_join(priority_by_stressor, by = c('iucn_sid', 'stressor')) %>%
  filter(term == 'year') %>%
