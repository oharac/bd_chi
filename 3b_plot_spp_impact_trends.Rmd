...
Here we will not summarize by every stressor, but just the cumulative category levels.

``` {r}
priority_by_stressor <- imp_range_df %>%
  filter(!is.na(year)) %>%
  filter(str_detect(stressor, 'cum_')) %>%
  group_by(stressor, iucn_sid, qtile) %>%
  filter(year == max(year)) %>%
  summarize(priority = pct_impact / log(range_km2),
            impact_yr = year) %>%
  ungroup()
```
