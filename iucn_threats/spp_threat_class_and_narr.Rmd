...
spp_threats_file <- file.path(dir_o_anx, 'iucn/threats',
                              sprintf('iucn_spp_threats_%s.csv', api_version))

spp_info <- read_csv(file.path(dir_data, sprintf('spp_marine_from_api_%s.csv', api_version)))
...
if(!file.exists(spp_threats_file)) {
  ...
  for(j in 1:n_chunks) { 
    ...
    chunk_file <- file.path(dir_o_anx, 'tmp', 
                            sprintf('spp_threat_chunk_%s_%s.csv', 
                            min(spp_index), max(spp_index)))

    if(!file.exists(chunk_file)) {
      cat_msg('Getting threat info for species ', min(spp_index), ' to ', max(spp_index))
      
      spp_ids_chunk <- spp_ids[spp_index]
      spp_threat_chunk <- mc_get_from_api(threat_url, spp_ids_chunk, api_key, 
                                          delay = .5, cores = 16, verbose = FALSE)
      cat_msg('... found ', nrow(spp_threat_chunk), ' threat rows for these species')
      
      write_csv(spp_threat_chunk, chunk_file)
      
    } else {
      
      cat_msg('Chunk file ', basename(chunk_file), ' already exists; skipping these spp')
      
    }
  }
  
  spp_threat_chunk_files <- list.files(file.path(dir_o_anx, 'tmp'), 
...
spp_info <- read_csv(file.path(dir_data, sprintf('iucn_risk_current_%s.csv', api_version)))

spp_narr_file <- file.path(dir_o_anx, 
                           sprintf('iucn/spp_narr_from_api_%s.csv', api_version))
reload <- FALSE

if(!file.exists(spp_narr_file) | reload) {
  
  cat_msg('Using API to determine species narrative from marine species info list')
