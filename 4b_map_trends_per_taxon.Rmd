      ...
      if(length(spp_ids_taxa) == 0) {
        ...
        for(i in 1:n_subgps) {
          ...
          trend_gp_summary <- trend_gp %>%
            ...
            group_by(cell_id) %>%
            summarize(priority_incr_spp = sum(priority * incr, na.rm = TRUE),
                      priority_decr_spp = sum(priority * decr, na.rm = TRUE),
                      n_incr_spp = sum(incr, na.rm = TRUE),
                      n_decr_spp = sum(decr, na.rm = TRUE)) %>%
            ungroup()
...
for(str_cat in strs) {
  ...
  for(t in taxa) {
    ...
    incr_map <- raster(sprintf(fstem, t, str_cat, 'incr'))
    plot(incr_map, 
         col = c('grey70', hcl.colors(n = 20, rev = TRUE)),
         main = sprintf('intensifying %s impacts, %s', str_cat, t))
    
    decr_map <- raster(sprintf(fstem, t, str_cat, 'decr'))
    plot(decr_map, 
         col = c('grey70', hcl.colors(n = 20, rev = TRUE)),
