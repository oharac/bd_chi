...
write_csv(threats_to_maps, here('int/spp_threats_fishing.csv'))
...
gear_narr_class <- gear_narr_df %>%
  mutate(dredge_count   = str_count(sentence, 'dredge'),
         ...
         gillnet_count  = str_count(sentence, 'gill.?net'),
...
art_fish_from_iucn <- spp_threats_from_iucn %>%
  ...
  mutate(sensitive = as.logical(sens)) %>%
  select(iucn_sid, stressor, sensitive) %>%
  distinct()
...
gear_sens_summary <- gear_sens_df %>%
  ...
  filter(!is.na(sens)) %>%
  summarize(sensitive = sum(count * sens) > 0) %>%
...
```{r}
n_sens_spp <- gear_sens_summary %>%
  filter(sensitive) %>%
  .$iucn_sid %>%
  unique()
```

According to this analysis so far, there are `r length(n_sens_spp)` distinct species affected by various fishing stressors.
