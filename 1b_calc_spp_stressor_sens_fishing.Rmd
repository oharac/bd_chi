...
gears_midtrawl <- c('mid.?water trawl', 'pelagic trawl', 'entangl')
...
trawl_narr_class <- gear_narr_df %>%
  mutate(trawl_count = str_count(sentence, 'trawl|danish seine'),
...
gear_narr_class <- gear_narr_df %>%
  mutate(dredge_count   = str_count(sentence, 'dredge'),
         ...
         gillnet_count  = str_count(sentence, 'gill.?net|entangl'),
         midtrawl_count = str_count(sentence, paste0(gears_midtrawl, collapse = '|')),
         btmtrawl_count = str_count(sentence, paste0(gears_btmtrawl, collapse = '|'))) %>%
  select(-sentence) %>%
  full_join(trawl_narr_class, by = c('iucn_sid', 'dimension')) %>%
  ### check that the midtrawl and btmtrawl counts don't exceed the 
  ### total non-scientific/exploratory trawls
  mutate(midtrawl_count = ifelse(midtrawl_count > trawl_count, 
                                 trawl_count, midtrawl_count),
         btmtrawl_count = ifelse(btmtrawl_count > trawl_count, 
                                 trawl_count, btmtrawl_count)) %>%
  ### if trawl counts but no mid- or btm-trawl, assign trawl counts to btm_trawl
  ### (assume generic trawl means bottom)
  mutate(btmtrawl_count = ifelse(midtrawl_count == 0 & btmtrawl_count == 0,
                                 trawl_count, btmtrawl_count)) %>%
  select(-trawl_count)
...
write_csv(gear_sens_summary, 
          here('_output', sprintf('spp_sens_fishing_%s.csv', api_version)))
```
