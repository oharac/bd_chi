...
|  11.1 | Habitat shifting & alteration                 | sst, slr, oa, uv | 
|  11.2 | Droughts                                      | NA               | 
|  11.3 | Temperature extremes                          | sst              | 
|  11.4 | Storms & flooding                             | NA               | 
|  11.5 | Other impacts                                 | oa, uv           | 
...
write_csv(threats_to_maps, here('int/spp_threats_climate.csv'))
...
For fishing stressors, we differentiated between demersal and pelagic to predict likely impacts from different gear types.  Here we examine shallow vs. deep species to help parse potential sensitivity to sea surface temperature and UV exposure, and species who rely on intertidal and shore to parse sensitivity to sea level rise.  
...
oa_search_string <- 'ocean acidification'
...
cc_narr_df <- spp_narrs %>%
  filter(dimension == 'threats') %>%
  tidytext::unnest_tokens(input = text, output = trigram, 
                          token = 'ngrams', n = 3) %>%
  filter(str_detect(trigram, cc_keywords)) %>%
  mutate(oa_count  = str_count(trigram, oa_keywords),
         slr_count = str_count(trigram, slr_keywords),
         sst_count = str_count(trigram, sst_keywords)) %>%
  select(-trigram)
...
write_csv(cc_sens_summary, 
          here('_output', sprintf('spp_sens_climate_%s.csv', api_version)))
```
...
```{r}
n_sens_spp <- cc_sens_summary %>%
  filter(sens) %>%
  .$iucn_sid %>%
  unique()
```

According to this analysis so far, there are `r length(n_sens_spp)` distinct species affected by various climate stressors.
