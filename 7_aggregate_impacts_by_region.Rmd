...
Using the impact category level global maps, aggregate species impacts to country level.  We also aggregate to marine ecoregion level.
...
get_impact_summary <- function(f, rgn_df, rgn = 'eez') {
...
impact_cats <- c('fishing', 'land-based', 'ocean', 'climate', 'all')
rast_files <- list.files(here('_output/rasters'), pattern = '.tif$', 
                         full.names = TRUE)
### filter to those in the impact categories
rast_files <- rast_files[str_detect(rast_files, paste0(impact_cats, collapse = '|'))]

count_rasts    <- rast_files[str_detect(rast_files, 'impact_map')]
priority_rasts <- rast_files[str_detect(rast_files, 'impact_priority_map')]
### for trend rasts, get the value (not the p_val) raster
trend_rasts    <- rast_files[str_detect(rast_files, 'trend_.+_value.tif')]
```
...
if(!file.exists(impact_count_by_eez_file)) {
  ...
  impacts_by_eez_list <- parallel::mclapply(count_rasts,
...
if(!file.exists(impact_pct_by_eez_file)) {
  ...
  nspp_rastfile <- here('_output/rasters/n_spp_map.tif')
  spp_by_eez_df <- get_impact_summary(f = nspp_rastfile,
                                      rgn_df = rgn_df, rgn = 'eez') %>%
    select(eez, nspp_mean = val_mean, nspp_var = val_var)

  imp_nspp_df <- impacts_by_eez_df %>%
    full_join(spp_by_eez_df, by = 'eez') %>%
    mutate(pct_impact_mean = impact_mean / nspp_mean * 100) %>%
    select(eez, pct_impact_mean, a_km2, year, impact)
