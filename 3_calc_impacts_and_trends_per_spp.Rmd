...
This table checks the trends across each stressor map, calculates a high threshold (at the 80th percentile) and low threshold (zero).  For "high" trends and "low" trends, what proportion of these cells show a statistically significant trend?
...
for(i in seq_along(stressors)) {
  ...
  cutoff <- quantile(df$trend, .8)
  df2 <- df %>%
    mutate(range = case_when(trend > cutoff ~ 'high', 
                             trend < 0 ~ 'low',
                             TRUE ~ 'mid'),
           sig = p < 0.05) %>%
    group_by(range) %>%
...
knitr::kable(check_df)

```

Reclassify trend values into pace

* For per-cell trend value $T$ for each stressor, determine the threshold $T_{80th}$ from 80th percentile across all cells.
* For each cell, reclassify $T$ (linear model slope) into $P$ (categorical):

\begin{align*}
  P = \begin{cases} 
    +1 &\text{if } (T \geq T_{80th}) * (T > 0) * p < 0.05\\
    -1 &\text{if } (T < 0) * (p < 0.05)\\
    NA &\text{else}
  \end{cases}
\end{align*}
...
pace_file <- file.path(dir_bd_anx, 'layers/stressor_pace.csv')
# unlink(pace_file)

if(!file.exists(pace_file)) {
  trend_fstem <- file.path(dir_str_trend,
                           'stressor_trend_%s_value.tif')
  p_val_fstem <- file.path(dir_str_trend,
                           'stressor_trend_%s_p_val.tif')
  pace_df <- parallel::mclapply(stressors,
      FUN = function(str) { ### str <- stressors[1]
        t_r <- raster(sprintf(trend_fstem, str))
        p_r <- raster(sprintf(p_val_fstem, str))
        ### calculate high cutoff as 80%ile value.
        cutoff <- quantile(values(t_r), 0.8, na.rm = TRUE)
        
        df <- rast_to_map(t_r, cell_val = 'trend') %>%
          full_join(rast_to_map(p_r, cell_val = 'p')) %>%
          mutate(pace = case_when(trend >= cutoff & trend > 0 & p < 0.05 ~ 1,
                                  trend < 0 & p < 0.05 ~ -1,
                                  TRUE                 ~ NA_real_),
                 stressor = str) %>%
          filter(!is.na(pace))
...
calc_cum_impact <- function(spp_id, impacts, pace_df) {
  ...
  spp_range_km2 <- get_spp_range(id = spp_id)
  ...
  spp_impact_by_stressor <- spp_intsx_df %>%
    ...
    summarize(impact_km2 = sum(ocean_prop) * cell_area_km2,
              incr_km2   = sum((pace == 1) * ocean_prop, na.rm = TRUE) * cell_area_km2, 
              decr_km2   = sum((pace == -1) * ocean_prop, na.rm = TRUE) * cell_area_km2) %>%
    ungroup()
  
  spp_cum_impact <- spp_intsx_df %>%
    # select(cell_id, year, ocean_prop, stressor) %>%
    distinct() %>%
    group_by(cell_id, year, ocean_prop) %>%
    summarize(incr = any(pace == 1, na.rm = TRUE) & !any(pace == -1, na.rm = TRUE),
              decr = any(pace == -1, na.rm = TRUE) & !any(pace == 1, na.rm = TRUE),
              n_impact = n()) %>%
    group_by(year) %>%
    summarize(impact_km2 = sum(ocean_prop) * cell_area_km2,
              impact_2plus_km2 = sum((n_impact > 1) * ocean_prop) * cell_area_km2,
              impact_3plus_km2 = sum((n_impact > 2) * ocean_prop) * cell_area_km2,
              incr_km2   = sum(incr * ocean_prop, na.rm = TRUE) * cell_area_km2, 
              decr_km2   = sum(decr * ocean_prop, na.rm = TRUE) * cell_area_km2,
              stressor = 'cumulative')
  ...
  if(nrow(spp_cum_impact) == 0) {
    spp_cum_impact <- data.frame(
      stressor = 'cumulative',
      year = NA,
      impact_km2 = 0, incr_km2 = 0, decr_km2 = 0)
  }
