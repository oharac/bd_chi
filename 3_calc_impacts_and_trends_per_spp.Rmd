...
if(!file.exists(pace_sum_file)) {
  ...
  for(i in seq_along(stressors)) {
    ...
    df2 <- df %>%
      mutate(range = case_when(trend >   0.01 ~ 'rapid_incr',
                               trend <  -0.01 ~ 'rapid_decr',
                               trend >  0.001 ~ 'incr',
                               trend < -0.001 ~ 'decr',
                               trend > 0 ~ 'small_incr', 
                               trend < 0 ~ 'small_decr',
  ...
  check_df <- bind_rows(check_list) %>%
    ...
    mutate(range = factor(range, levels = c('rapid_incr', 'incr', 'small_incr', 'small_decr', 'decr', 'rapid_decr'))) %>%
    filter(!is.na(range))
  ...
  area_df <- bind_rows(check_list) %>%
    ...
    # mutate(incr = incr + rapid_incr,
    #        small_incr = small_incr + incr,
    #        decr = decr + rapid_decr,
    #        small_decr = small_decr + decr) %>%
    select(stressor, rapid_incr, incr, small_incr, small_decr, decr, rapid_decr)
  
  write_csv(check_df, pace_sum_file)
}
  
check_df <- read_csv(pace_sum_file) %>%
  mutate(range = factor(range, levels = c('rapid_incr', 'incr', 
                                          'small_incr', 'small_decr', 
                                          'decr', 'rapid_decr')))
