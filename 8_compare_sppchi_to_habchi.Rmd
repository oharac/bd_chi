...
* Pull in original data (CHI layers from 2011-2013)
* Calculate mean CHI over this span
...
if(!file.exists(mean_10km_file)) {
  ### Step 1 and 2: read in orig res, find mean
  mean_1km_file <- file.path(dir_bd_anx, 'tmp/chi_mean_1km.tif')
  if(!file.exists(mean_1km_file)) {
    chi_rast_stem <- file.path('/home/shares/ohi/git-annex', 
                       'impact_acceleration/impact', 
                       'cumulative_impact/chi_%s.tif')
  
    chi_stack <- stack(sprintf(chi_rast_stem, 2011:2013))
  
    chi_mean <- calc(chi_stack, fun = mean, na.rm = TRUE, 
                     # filename = file.path(dir_bd_anx, 'tmp/chi_mean_1km.tif'),
                     overwrite = TRUE)
  } else {
    chi_mean <- raster(mean_1km_file)
  }
  
  ### step 3: aggregate to ~10 km resolution
