...
We will also map species impacts using the priority weights from script 3b, i.e.
...
priority_df <- read_csv(here('_output/spp_priority_by_stressor.csv')) %>%
  mutate(priority = priority / max(priority)) %>%
  select(iucn_sid, stressor, priority)

for(str_cat in str_cats) {
  # str_cat <- str_cats[1]
  
  pr_str_df <- priority_df %>%
    filter(stressor == paste0('cum_', str_cat))
  ...
  for(taxon_gp in taxon_gps) {
    ...
    if(any(!file.exists(taxon_map_files, taxon_priority_map_files)) | reload) {
      ...
      if(length(spp_ids_taxa) == 0) {
        ...
        for(i in 1:n_subgps) {
          ...
          impacts_gp <- impact_maps %>%
            dt_join(pr_str_df, by = 'iucn_sid', all = FALSE) %>%
            group_by(cell_id, year) %>%
            summarize(priority_sum_spp = sum(priority),
                      n_spp = n_distinct(iucn_sid)) %>%
            ungroup()
...
## Animate count vs. priority maps

Just for the most recent year of each, back and forth to highlight diffs.  To highlight the differences in _pattern_ not _magnitude_, I'll stretch the priority scale to have the same range as the count scale (i.e. 1 to the max number of critters).

``` {r animate impacts v priorities, results = 'asis', eval = TRUE}
### Animate the results
